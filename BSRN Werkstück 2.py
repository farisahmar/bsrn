...
class Process:
    def __init__(self, name, burst_time, arrival_time):
        # Initialisierung der Prozessattribute
...
def read_processes_from_file(file_path):
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            for line in lines:
                parts = line.split()  # Aufteilen der Zeile in einzelne Teile
                if len(parts) == 3:
...
def prozesse_in_warteschlange_hinzufuegen(aktuelle_zeit, prozesse, warteschlangen, hinzugefuegte_prozesse, log_file):
    for prozess in prozesse:
        if aktuelle_zeit >= prozess.arrival_time and prozess not in hinzugefuegte_prozesse:
            ...
            warteschlangen[0].append(prozess)  # Hinzufügen des Prozesses zur ersten Warteschlange
...
def round_robin_scheduling(warteschlangen, quantum, prozesse, hinzugefuegte_prozesse, log_file):
    ...
    timeline_chart = []  # Liste zur Speicherung der Timeline-Daten
            ...
            if warteschlange:
                ...
                for _ in range(current_quantum):
                    if prozess.remaining_time > 0:
                        ...
                        timeline_chart.append((aktuelle_zeit, prozess.name, i + 1))  # Hinzufügen der Timeline-Daten
                ...
                if prozess.remaining_time > 0:
                    ...
                    prozess.turnaround_time = aktuelle_zeit - prozess.arrival_time  # Berechnung der Durchlaufzeit
                ...
                break  # Schleife für die Warteschlangen beenden, wenn ein Prozess gefunden wurde
...
# Funktion zum Schreiben der Timeline-Diagramme in eine PNG-Datei
def schreibe_timeline_diagramm(timeline_chart, prozesse, timeline_file_path):
    ...
    for queue, color in zip(unique_queues, colors):
        warteschlange_farbe[queue] = color  # Zuordnung der Farben zu den Warteschlangen
    ...
    plt.figtext(0.5, -0.1, info_text, ha="center", fontsize=12, wrap=True)  # Hinzufügen des Informationstexts zur Figur
...
def schreibe_timeline_txt(timeline_chart, prozesse, timeline_file_path):
    with open(timeline_file_path, 'w') as file:
        ...
        file.write(f"Durchschnittliche Gesamtlaufzeit: {avg_turnaround_time}\n")
...
def schreibe_logdatei(log_file_path, warteschlangen_gesamt, quantum, file_path, timeline_file_path, processes):
    with open(log_file_path, 'w') as log_file:
        ...
        log_file.write(f"Quanten pro Warteschlange: {quantum}\n")  # Schreiben der Quanten pro Warteschlange in die Logdatei
        ...
        timeline_chart = round_robin_scheduling(warteschlangen, quantum, processes, hinzugefuegte_prozesse, log_file)  # Durchführung des Round-Robin-Schedulings
...
def main():
    ...
    parser.add_argument('-processlistfile', type=str, help="Pfad zur Eingabedatei mit den Prozessen")
    parser.add_argument('-logdatei', type=str, help="Pfad zur Log-Datei")
    ...
    if args.warteschlangen and args.quantum and args.processlistfile and args.logdatei and args.ausgabeformat and args.ausgabedatei:
        ...
        output_format = args.ausgabeformat
    ...
    # Durchführung des Round-Robin-Schedulings und Schreiben der Logdatei
